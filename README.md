...
| 方法-检验员资质导入模板 | **方法资质**页 → 「下载模板」 |
| 项目标准库导入模板 | **项目标准库**页 → 「下载模板」 |
| 检验员任务 Excel | **我的检验任务**页 → 「下载任务 Excel」（结果录入页也有「导出我的任务」） |
| 省平台上报数据 Excel | **省平台上报**页（可用任务编号筛选） |
| 检验报告（纸质） | **报告生成**页 → 生成/重打印 → 打印页浏览器「打印 / 另存为 PDF」 |

### 下载会存到哪里？

**所有下载都会弹出「另存为」对话框，由你自己选择保存位置与文件名**（2026-09-14 起）。
对话框中的建议文件名带时间戳，例如 `检验任务20260914162006.xlsx`、`省平台上报数据20260914162012.xlsx`。

两种情况会退回到「浏览器默认下载目录」（通常是 `C:\Users\<你>\Downloads`），并给出提示：

1. 浏览器不支持 File System Access API（Firefox / Safari）；
2. **页面不是通过 `localhost` / `127.0.0.1` / HTTPS 打开的**——该 API 要求安全上下文。
   例如通过 `http://192.168.1.x:5173` 这种内网 HTTP 地址访问时，会自动降级为默认下载目录。

> 检验报告**不产出文件**：报告是白底 A4 的 HTML 页面，打印即得纸质件；
> 需要电子件时在打印对话框里把目标选成「另存为 PDF」，保存位置同样由你选择。
