...
| Copilot CLI | `agent/copilot` | **可选复核**（契约复核、规则复核、diff 复核）——**默认不参与，不阻塞主线** | auto 模型 |
...
- 结果自动判定规则（见 7.3）只能由 GLM 实现于后端，检验员不可手改单项结论，前端不可自算结论；判定口径的歧义由 **GLM 自行裁决**（见 2.6），裁决须附证据并落档 DECISIONS.md。
...
- **契约先行**：任何需要前后端联调的功能，GLM 必须先把接口定义写入 `docs/api/api-spec.md`（路径/方法/请求/响应/权限标识）；**GLM 自审并落档**，Copilot 复核为可选项（见 2.6）。全员以该契约为唯一依据，禁止自创接口格式。
...
- **Copilot 的角色（2026-09-11 二次调整后：可选复核，不阻塞主线）**：
  1. **契约复核**（可选）：对 GLM 起草的接口定义做一致性复核（字段命名/路径风格/响应结构/权限标识）；
  2. **规则复核**（可选）：对 GLM 自裁的判定口径做复核（见 2.6）；
  3. **diff 复核**（可选）：对合并进 develop 的代码做审查。
  > ⚠️ 以上三项**默认不发起**。仅在 GLM 自裁遇到「真实两难 + 影响不可逆」时才启用（裁决配额上限见 2.6）。
- 分工边界：**GLM 实现并自裁，Copilot 可选复核**。Copilot 不承接 S/A 级实现类任务。
- 降级：GLM5.3 额度不足 → 任务拆小，体力部分转豆包；切 HY4 preview 后只做纯 CRUD，S/A+ 任务排队；**严禁为省额度让低能力模型做 S 级任务**（返工成本 > 省下额度）。

### 2.4 冲突处理

- 冲突必须由涉及双方代码的 Owner 共同核对，禁止强推覆盖；
- 公共文件冲突一律以 GLM 版本为基准，他人重放自己的增量；契约（api-spec.md）冲突以**最新落档版本**为基准，存在争议时按 2.6 自裁决流程处理。
...
### 2.6 自行裁决机制（⚠️ 2026-09-11 用户决策：GLM 自裁，不再等待 Copilot）

> **用户原话**：「接下来你尽量不依赖 copilot 裁决完成项目，方便我启动一次完整项目测试……同时也方便你重整思路。」

**规则**：

1. **默认自裁**：判定口径、数据形态歧义、跨模块语义冲突，一律由 **GLM 自行裁决**，不得挂起等待 Copilot。
2. **自裁的证据标准（不得低于原 Copilot 裁决定稿）**，必须同时满足：
   - **可复现证据**：SQL 探针结果、数据分布统计、代码/契约引用，写进决策条目；
   - **落档**：技术决策写 `DECISIONS.md`；涉及判定口径写 `docs/knowledge/`（覆盖式更新，不新开草案）；
   - **反例说明**：写明「排除了哪些备选方案、为什么」；
   - **fail-loud 优先**：凡「无法判定」的情形，一律落到「待判定 + 日志」，**禁止静默判合格**。
3. **Copilot 裁决降为可选兜底**：仅在「自裁存在真实两难 **且** 影响不可逆（如已出报告的结论口径）」时启用。
   - 配额：**全项目上限 2 次，已用 1 次（T-905），剩余 1 次**。启用前须先尝试自裁并留下尝试痕迹。
4. **禁止把「等 Copilot」当作推进阻塞**：任何任务不得因等待裁决而停在 TODO。
5. **自裁同样受 2.5 三件套约束**：日记中必须记录「本轮的裁决点是什么、依据是什么」。

**判定「真实两难」的三问**（三问全「是」才考虑动用配额）：
- 该歧义是否**无法**通过数据/契约/标准文本得到唯一答案？
- 两种口径是否会导致**已交付产物需要返工**（而非仅新增字段）？
- 是否已有**监管/资质**含义（CMA/CATL 报告结论）？

### 2.7 豆包协作分工（杂事委派清单）

> 用户要求：「期间杂事你可以交给豆包处理」。目的是让 GLM 专注 S/A 主线。
> 权威任务状态仍以 `TODO.md` 为准，本节只界定「什么算杂事」。

**可直接委派豆包（B 级）**：
- 文档整理与排版：`README.md`、`docs/database-dictionary.md`、HANDOFF/STATUS 的**格式**维护；
- 数据类：`db/seed/` 测试数据、演示数据、省平台上报模板（T-802）；
- 资产类：报告模板版式（T-702 的**版式样本整理**，非引擎实现）、截图与界面文案核对；
- 校对类：字段命名一致性核对、契约与 Entity 的**逐字段比对**（发现不一致提 TODO，不自行改代码）；
- 环境类：`db/init/*` 按序执行与结果记录、迁移脚本执行前后的条数校验 SELECT 记录。

**禁止委派豆包**：
- 任何 `backend/` 代码、状态机流转、判定引擎相关逻辑；
- `db/init/`、`db/migrations/` 的**内容设计**（可执行与记录，不可设计）；
- `docs/api/api-spec.md` 契约内容修改；
- 裁决类工作（口径判定）。

**委派方式**：在 `TODO.md` 建条目并标 Owner=豆包，写明「输入 / 输出 / 验收标准」三要素；GLM 收工时 @豆包。
...
│   └── seed/                 # 测试数据（豆包维护）
├── docs/
│   ├── api/api-spec.md       # 接口契约（GLM 起草并自审落档；Copilot 复核可选）
...
### 5.1 UI 设计基准（⚠️ 2026-09-11 用户要求：对齐 Mineradio 级视觉品质）

- **主题名**：`Aurora Glass`（暗色沉浸 + 极光配色 + 折射玻璃）。
- **权威文件**：`docs/knowledge/2026-09-11-ui-design-mineradio-research.md`（设计语言与适配决策，开工必读）。
- **令牌**：`frontend/src/styles/tokens.css`（`--lims-*`）；**禁止硬编码色值**，一律走变量。
- **三类表面，用途不可混用**：

  | 类名 | 用途 | 特性 |
  |---|---|---|
  | `.lims-glass` | 卡片、外壳、弹层 | 玻璃渐变 + 内发光 + 模糊 |
  | `.lims-glass-refract` | **仅**「第一眼表面」（登录卡、品牌区、抽屉头） | 追加 SVG 位移折射 + RGB 色差 |
  | `.lims-panel` | **表格 / 表单等数据密集区** | 高对比、弱化模糊，保证长时间录入可读 |

- **硬性红线**：
  1. **数据密集区禁用折射玻璃**（强模糊会降低小字号数字可读性）；
  2. 强调色（`--lims-accent` 青）**只做光晕/描边/激活条**，按钮底色用 `--lims-brand-gradient`（保证白字对比度）；
  3. 动效预算 ≤ `0.45s`，且必须支持 `prefers-reduced-motion`；
  4. **参考第三方 UI 必须先查许可**：GPL/AGPL 项目只能借鉴设计思路与参数关系，**禁止逐字拷贝其代码/资源**。
...
任何 Agent 交付前必须通过本地质量门禁；合入 develop 前由 GLM 自行 diff 审查（范围见 2.3），Copilot 复核为可选项。
...
**Copilot（可选复核，默认不参与、不阻塞主线；2026-09-11 用户决策）**：在 GLM 发起复核请求时，对 `docs/api/api-spec.md` 做一致性复核、对 GLM 自裁的判定口径做复核、对 diff 做审查；复核不通过须在 TODO 退回并写明原因。**不得作为推进阻塞**（见 2.6）。
