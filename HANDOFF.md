...
你反馈的「问 GB 2762 铅的限量一直卡在正在生成，关掉再打开才看到回答」是**三个独立缺陷叠加**，已全部修复并实测闭证；
顺带把你这次问法暴露出的**检索召回错误**（问 GB 2762 却返回 GB 2763 的条款）也修掉了。

### 三个根因（都有实测证据，详见 `docs/knowledge/2026-09-18-sse-async-vue-reactivity-ngram-routing.md`）

| # | 根因 | 证据 | 修法 |
|---|---|---|---|
| 1 | `SseEmitter` 完成触发 **ASYNC 派发**→过滤器链重跑，而 JWT 过滤器默认跳过 ASYNC ⇒ 匿名 ⇒ `AccessDeniedException`；SSE 响应已提交无法转错误 ⇒ **连接被 RST** | `curl -N` **退出码 18**（帧其实发全了）+ 日志 `AuthorizationFilter:98` | 放行 `DispatcherType.ASYNC/ERROR` + `shouldNotFilterAsyncDispatch()→false` |
| 2 | **前端改「push 进 `ref([])` 的裸对象」不触发视图更新**（这才是「重开面板才看到答案」的真因） | Node + `@vue/reactivity` effect 计数：`render#` 停在 2 不动 | push 后**取回数组里的响应式代理**再改 |
| 3 | 整句走 BOOLEAN ⇒ 短语语义 0 命中；`GB`/`2763-2021` 噪声词 OR 兜底制造 **968 条假命中**，正确块未进候选 | MySQL 探针 `COUNT(*)` 与 `score=0` | 整句走自然语言模式 + 剔除噪声词 + 标题相关性优先排序 |

### 顺带修复

- **点名标准未收录 → 确定性诚实作答**：问 GB 2762（库内只有 GB 2763）不再返回 5 条无关的 GB 2763 条款；
- **OCR 条款归属错误**：`4. 121` / `毒死蜱（chlorpyrifos）` 两行式版式导致正文被挂到「表120」编号下 → 已支持合并；
- **审计口径**：流式接口此前记成 `operator=anonymousUser`、`duration_ms=0` → 现为 `nj001` / 真实耗时；
- **前端兜底**：SSE 加 30s 静默超时 + 「无 done 帧」显式回调，**界面不会再永久卡在「正在生成」**。

### 验收数据（自行复现，非采信子代理自述）

| 项 | 结果 |
|---|---|
| 后端 `mvn -o test` | **236 / 0 失败 / 0 错误 / BUILD SUCCESS** |
| 前端 `vue-tsc` / `eslint` / `vite build` | 0 / 0 / 成功 |
| 经 5173 代理的 SSE | `curl_exit=0`（修复前 18），三帧齐全 |
| GB 索引 | 4660 块、纯数字标题 **0**、`4.121 → 毒死蜱（chlorpyrifos）` |
| 检索抽样 5 项 | 条款全部排到第 1 位 |
| 审计日志 | `nj001` / `5570ms` |

### 仍待人工确认（未改）

- **浏览器端手感**（悬浮窗拖拽、引用卡片观感）需你实点；dev server 有 HMR，建议**硬刷新**一次页面；
- 4B 模型偶发把「表121」说成「表4.121」——**属模型措辞不精确，非结构问题**；数值一律不报（刻意设计）；
- `sys_operation_log` 里 `pageNum/pageSize`（旧分页域）与 `current/size`（新域）并存——历史约定，未改动。

## 2026-09-18 GLM → 用户（**AI 助手已打通真实链路，前端+后端+模型全部在跑，请测试**）

### 一句话

昨天交付的本地 AI 助手**今天跑通了真实链路**：你提供的 **67MB / 422 页扫描版 GB 2763-2021 已 OCR 并建成检索索引（1269 条条款）**，
助手现在能**自动关联标准、按样品状态给你下一步操作指引**；「全流程引导 + 自动查标准」两项新能力也已落地并实测。

### 🔌 现在可以直接测（三个服务都在跑）

| 服务 | 地址 | 说明 |
|---|---|---|
| **前端** | **http://localhost:5173** | 用 `nj001 / nj001` 登录（管理员视角能看全部功能） |
| 后端 | http://127.0.0.1:8080/api | 已启动（Tomcat，context-path `/api`） |
| 本地模型 | http://127.0.0.1:11434 | qwen3:4b-instruct 已就绪，自检 `online=true / latencyMs=4ms` |

> 日志都在 `_logs/`：`backend.log`、`frontend.log`、`ollama-restart.log`、`ocr-run2.log`。

### 建议按这个顺序测（每条都有实测依据）

1. **右下角悬浮窗**：点开 → 问「**S40 是什么意思？下一步该做什么？**」
   → 应**16ms 秒回**且是「请告诉我样品编号…」这类业务口吻（**不是**三星手机——模型原本会答成手机型号，业务域已改为确定性作答）。
2. **问标准**（点开助手，问）：「**毒死蜱在黄瓜上的限量是多少？**」
   → 应回：「依据 GB 2763-2021 4.121，毒死蜱…应符合表121的规定；具体限量请以系统标准库为准（可查看「数值对齐」卡片）」，
   并带 **5 条引用卡片**，卡片上有「扫描件OCR」标注。**它不会直接给你数字**——这是刻意设计（见下）。
3. **越界拒答**：问「**今天天气怎么样？**」→ 应礼貌拒答并给替代建议（不是红色报错）。
4. **流程引导**：在**结果录入页 / 项目分解页 / 任务安排页**点页头的「**下一步该做什么**」按钮
   → 打开流程引导：9 个阶段全部列出，含**下一步动作 / 可点跳转入口 / 所需权限 / 字段级提示**；
   你没权限的步骤会灰显并提示「需 X 权限，联系 R100/R2」。
5. **自动查标准（伴侣提示）**：在结果录入页选中带标准的项目 → 助手应弹**一行折叠建议条**「本条涉及 GB 2763-2021，查看对应限量出处」。
6. **AI 知识库页**：`/ai/kb`（菜单「AI 知识库」）→ 可检索 1269 条 GB 条款、查看导入任务进度。
7. **回退机制**（昨天交付，今天已在活库建表）：任取一个 S40 样品走「录入 → 保存 → 回退 → 改数据」，看「流程回溯」面板时间线。

### ⚠️ 必须知道的三件事（都是刻意的，不是 bug）

1. **助手绝不直接报限量数字**。原因有实证：模型凭记忆说「毒死蜱在黄瓜上 0.05 mg/kg」，
   而库里 `毒死蜱` 有 **25 条不同限量**（0.05/0.3/0.5/0.7/1/2/3 mg/kg，因为限量按**产品**区分），
   且检索出的原文里**黄瓜对应的是 0.02**——**模型答错了，而且错得看起来完全合理**。
   所以数值只从该系统标准库取（「数值对齐」卡片），AI 只做「引用与定位」。
2. **GB 标准是扫描件，OCR 必有错字**。实测把「最**大**残留限量」认成「最**人**残留限量」、`0.05` 变 `0. 05`。
   故 OCR 文本只用于条文引用，**不用于数值**；引用卡片会标注来源。
3. **4B 模型有偶然不一致**：同一问题换个问法，偶尔会回「标准条款未覆盖」。属可接受范围（助手定位是建议性引用），
   若影响体验可考虑换 7B 模型或加大 `lims.ai.max-context-chars`。

### 今天做了什么（含 3 个真 Bug 的修复）

| # | 事项 | 结果 |
|---|---|---|
| 1 | **补上活库缺失的 9 张表** | 昨天交付的两项能力在当前库里**一张表都没有**（当时我只在临时库验证过 DDL）→ 已建全 9 张表 + `deleted` 升 `bigint` + `void_status` + 8 个权限种子。**不做这步，你启动后只会看到报错** |
| 2 | **OCR 422 页扫描件** | 新增 OCR 通道（rapidocr-onnxruntime）；**修复进程空转活锁**（线程超订：4 worker × 32 线程抢 32 核 → 已钉成 4×7=28 线程）；断点续跑实测「已续跑 162 页、只补剩余 260 页」 |
| 3 | **🐛 导入任务永远「待处理」** | 根因：`@Transactional` 方法内直接调 `@Async` worker，**异步线程读不到未提交的 job 行** → 已改为「事务提交后投递」（3 处调用点） |
| 4 | **🐛 整句中文检索 0 命中** | 根因：BOOLEAN 模式下「不含空格的整句」被当短语（要求邻接）→ 实测同一整句布尔 0 条 / **自然语言模式 844 条** → 已补第三级兜底（AND → OR → 自然语言） |
| 5 | **🐛 模型把内部指令当答案输出** | 实测出现过「⚠️ 但请注意…不得凭记忆摘取数字…✅ 正确回答应为：」这种元话语外泄 → 已重写提示词（只给最终答复 + 给出作答模板） |
| 6 | **前端 5 处 TypeScript 错误** | 工程师在频率限制中断时留下的半成品（`assign/index.vue` 缺 import、`item/index.vue` 导了没调、`kb.vue` 类型不匹配）→ 已修 |

### 验收证据（真实命令输出）

| 项 | 结果 |
|---|---|
| 后端 `mvn -o test` | **223 / 0 failures / 0 errors / BUILD SUCCESS** |
| 前端三门禁 | `vue-tsc --noEmit` 0 错误 / `eslint` 0 error 0 warning / `vite build` 成功 |
| GB 索引 | `gb_document=1`、**`gb_clause=1269`**、导入任务「文件 1/1，条款 1269，失败 0」 |
| 真实问答 | 业务域 16ms 确定性作答；标准域 1.3s 带 5 条引用；越界拒答正常 |
| 流程引导 | 9 个阶段齐全，含 `entryPath`/`requiredPermission`/`hasPermission`/`fieldHint` |

### ⚠️ 未提交

**本轮没有执行任何 git 提交**——按你的要求，等你测试确认后再提交。
工作区同时含有**本会话之前的历史遗留未提交改动**，提交前必须 `git status --short` 逐项核对（AGENTS 第 12 章红线）。

### 已知遗留（不影响测试）

- 助手偶尔对同一问题给出「标准条款未覆盖」（4B 模型一致性）。
- `gb_document.std_title` 被写成了 `# source: GB2763-2021-ys.pdf（OCR 通道…）`——因为并档文本首行是来源标记，
  标题抽取把它当成了标题。**纯展示瑕疵**，检索不受影响。
- `product_lib_item` **没有食品类别列**，所以「某农药在某食品上的限量」无法从系统库唯一定位；
  这正是 OCR 原文的增量价值（原文里那张「食品类别 × 农药」二维表补上了这个维度）。
- OCR 侧车位 `ai/standards/.scan/`（422 个页文件）与 `_logs/` 保留未删，用于举证；两者都已在 `.gitignore`。

---

## 2026-09-17 GLM → 用户 / 下一棒（**新增两项能力：本地 AI 助手 + 全流程逐步回退**）

### 一句话

在已完工的 LIMS 上加装了**两项超出说明书范围的新能力**：一个**只谈本项目业务、能检索本地 GB 标准库**的
本地大模型悬浮助手（qwen3-4b，项目内托管），以及一套**覆盖七阶段每一步、可逐级回退并可取证**的流程回溯机制。
后端 **204 项单测全绿**，前端三门禁全绿；**但有两件事必须你亲手做才能闭环**（见下方「⚠️ 必须由你执行」）。

### 交付清单

| 类别 | 内容 |
|---|---|
| 设计与方案 | `docs/design/2026-09-17-prd-ai-assistant-and-rollback.md`（PRD，含 6 处设计张力裁决）<br>`docs/design/2026-09-17-arch-ai-assistant-and-rollback.md`（架构 1253 行：选型反例排除 + 9 张表 DDL + 接口契约 + 文件清单 + 任务分解） |
| 数据库 | 新增 `db/init/10_rollback_tables.sql`（4 表）、`db/init/11_ai_tables.sql`（5 表）、`db/migrations/V8__rollback_and_ai.sql`；<br>存量表语义变更：`06_item_tables.sql` / `07_result_tables.sql` 的 `deleted` 列 TINYINT→BIGINT；<br>权限种子 `db/seed/01_rbac_seed.sql`（AI 12x / 回溯 13x） |
| 后端 | 新增约 90 个类（回退域 + AI 域）；修改 `SampleStatusTransition`（+第三条白名单 `ROLLBACK`）与 6 个既有 ServiceImpl（接入状态流水） |
| 前端 | 新增悬浮窗 `components/ai/*`（6 件）、`components/rollback/RollbackTimelineDrawer.vue`、`views/rollback/index.vue`、`views/ai/audit.vue`、`stores/aiAssistant.ts`、`api/{ai,rollback}.ts`、`utils/aiStream.ts`；修改 `MainLayout.vue`、`router/*`、`report/audit.vue`、`result/index.vue` |
| AI 运行时 | `ai/README.md`、`ai/config/*`、`ai/scripts/{deploy-ollama,start-ollama,stop-ollama,status-ollama}.ps1`、`prepare-standards.py`、`import-standards.ps1`、`requirements.txt` |
| 经验资产 | `docs/knowledge/2026-09-17-rollback-mechanism.md`、`docs/knowledge/2026-09-17-local-ai-and-gb-retrieval.md`、`docs/journal/2026-09-17-glm-ai-assistant-and-rollback.md` |
| 决策 | `DECISIONS.md` 2026-09-17（架构师 10 条 + 交付总监 6 条，含 4 处推翻旧自裁） |
...
1. **先部署运行时**（**必须你执行**）：
   ...
   .\deploy-ollama.ps1      # 下载便携版运行时到 ai\runtime\、拉取 qwen3:4b 到 ai\models\（多镜像自动回退）
   .\start-ollama.ps1       # 启动（会把 OLLAMA_MODELS 指向仓库内目录）
   .\status-ollama.ps1      # 自检：运行时 / 模型 / API 三项
   ```
   > 设计要点：运行时与权重**全部在仓库内**（`ai/runtime`、`ai/models`），整个 `D:\lims` 可整体搬走；
   > 这两目录已在 `.gitignore` 中排除（体积大，不入库）。
2. **投放 GB 标准文件**：把标准文件放进 `ai/standards/inbox/`，PDF 先跑 `prepare-standards.py` 转文本（后端**拒收 PDF**），
   然后用 `import-standards.ps1` 一条命令建索引。检索走 MySQL `ngram` 全文索引，只查索引不读文件。
3. **在页面上用**：右下角气泡 → 展开悬浮窗；按住头部可拖到任意位置，位置会记住。

**能力边界（务必知道）**：
- 助手**只谈本项目业务与检验检测领域**，越界会**礼貌拒答**（同事口吻 + 给替代建议），不是报错。
- 助手对标准只做「**建议性引用**」（必带标准号 + 条款出处），**绝不给出合格/不合格结论**；
  判定永远以系统 `JudgeEngine` 为准。界面上有「AI 建议，仅供参考」的固定标注。
- **Ollama 没启动时**，助手面板会显示状态点与修复指引，**业务功能完全不受影响**（AI 是旁路）。

### 特性 B：全流程逐步回退（怎么用）

| 项 | 说明 |
|---|---|
| 入口 | 业务列表行「回退」、详情页按钮、独立「**流程回溯**」面板（时间线） |
| 能退哪些步 | `S20→S10`、`S30→S20`、`S40→S30`、`S50→S40`、`S60→S50`（常规）；`S70→S60`（**敏感**，需 R100/R2 + 强理由 + 二次确认） |
| 不能退哪些步 | **S80 已签发 / S90 已出报告 —— 白名单里根本没有这条边**。改用 `report_void`（作废/召回**标记动作**，只打标记不改状态，保留原签发记录） |
| 回退前 | 弹出确认框：当前状态 → 目标状态、**将被失效的下游数据计数与清单**、**原因必填**、是否需要二次确认、不可逆提示 |
| 回退后 | 历史**不消失**：下游数据置失效 + 留档（`sample_data_archive` 存整行快照可取证）；每次回退在 `sample_status_log` 追加一条事件；未产生新下游数据的回退**可原路恢复** |
| 与「审核退回」的关系 | 同一条统一链路与留痕，但语义不同：**退回**=质量否定（审核人打回）/ **回退**=数据订正（操作者撤回）/ **作废**=报告治理。文案与视觉在界面上区分 |

### 关键设计（给后续维护者，勿违反）

1. **状态机有三张独立白名单**：`VALID`（正向）/ `RETURN`（审核退回）/ **`ROLLBACK`（纠错回退）**，
   **互不调用、不得合并**。新增回退边只能进 `ROLLBACK`。
2. **回退四条不变式**（已用测试固化，不是注释）：① 乐观条件 UPDATE 冲突→业务码 **4108**；
   ② 失效处置必须在状态 UPDATE **之后**、**同一事务**；③ 一次回退在 `sample_status_log` **仅新增 1 条** `event_type=4`；
   ④ **不存在物理删除路径**。
3. ⚠️ **`sample_item` / `sample_result` 的 `deleted` 语义已升级**：`0`=有效，**非 0 = 该行自身 id**。
   目的：让逻辑删除可**重复执行**（解决「覆盖式重建 + 二次失效撞唯一键」）。
   **MyBatis-Plus `@TableLogic` 只支持固定字面量、不支持表达式**（实测），
   故这两张表的失效必须走 `SampleDataDisposer` 显式 `set(deleted, id)`，**禁用 `baseMapper.delete(...)`**。
   其余表仍是 `0/1` 两态，全局 `logic-delete-value: 1` **保留不变**。
   **维护红线**：全仓不得再出现 `eq("deleted", 1)` / `deleted == 1` 这类硬编码判定（新语义下失效值 ≠ 1）。
4. **AI 不碰判定链**：`service/ai/**` 任何类都不得注入能写 `sample_info.conclusion` /
   `sample_result.conclusion` / `judge_basis` 的 Mapper。`AiDomainIsolationTest` 用反射穷举依赖守住这条红线。
5. **GB 索引表（`gb_document`/`gb_clause`）允许物理重建**——它们是**可再生的派生索引**，不是业务留档，
   不适用「历史不消失」约束（换版标准时物理 DELETE 再重建是正确做法）。
6. **`OperationLogInterceptor` 的 `MODULE_PREFIXES` 是有序 `LinkedHashMap`**：新增前缀要注意顺序语义
   （`/report/audit` 必须排在 `/report` 之前的历史教训，已有单测固化，新增 `/ai/kb` 同理须在 `/ai` 之前）。
7. **`autoRegister` 是窄例外**：允许「会话审计」页由权限驱动注册、侧栏不出现（`kb.vue` 头部提供入口）。
   **移除条件**：seed `sys_menu` 补出该页菜单节点后立即删除该用法。**不得新增第二个 `autoRegister` 用法**（见 DECISIONS 2026-09-17）。

### ⚠️ 必须由你执行才能闭环（**不得视为已完成**）

1. **Ollama 运行时与 qwen3:4b 权重尚未下载**。实施环境（沙箱）代理对大文件返回 502，
   实测 **7 条镜像路径全部失败**（GitHub 直连 / ghfast.top / gh-proxy.com / ghproxy.net / gh.llkk.cc / hf-mirror / modelscope）。
   本轮 AI 能力的验收边界因此是「**Ollama 不可用时系统行为正确**」（降级 + fail-loud + 业务零影响），
   **真实推理链路未端到端跑通**——请在本机执行 `ai/scripts/deploy-ollama.ps1` 后自行验证一次问答。
2. **GB 标准库无真实数据**（你尚未提供标准文件），检索链路目前只用构造数据验证过。
3. **UI 未做真实浏览器人工体验终验**：悬浮窗拖拽手感、位置记忆、回退确认框与时间线的实际观感，请点一次。

### 验收证据（真实命令，非自述）

| 项 | 结果 |
|---|---|
| 后端 `mvn -o test` | **212 项全绿**（既有 151 + 新增 61，0 失败 0 错误，BUILD SUCCESS）。首轮交付 204；QA 第 2 轮新增 `SampleDataDisposerTest`(8) 把**不变式④「无物理删除路径」**固化成断言（行为层穷举 Mapper 调用无 `delete*`/`remove*` + 反射断言无 `@Delete`），并加固乐观 UPDATE 的 `WHERE status=旧值` 捕获 |
| 前端 `vue-tsc --noEmit` / `npm run lint` / `npm run build` | 三门禁全绿 |
| 三条白名单独立性 | 独立复核确认：`ROLLBACK` 恰好 6 条边、无 S80/S90 出边；`assertTransition(S60,S50)` 仍拒绝 |
| AI 不碰判定链 | `service/ai` 对结论字段 grep **零命中** + `AiDomainIsolationTest` 反射断言 |
| 回退四条不变式 | `InOrder` + `verify(times(1))` 固化 |

### 未提交

本轮**未执行任何 git 提交**（本机 git 沙箱有已知问题，统一后续处理）。
工作区含**本会话之前的历史遗留未提交改动**（如 `directives/permission.ts` 09-14、
`.agents/skills/excel-import/SKILL.md` 09-15）。
**提交前必须 `git status --short` 逐项核对**（AGENTS 第 12 章红线，4070ea6 误删事故的教训）。

### 下一步建议顺序

1. 跑 `ai/scripts/deploy-ollama.ps1` → `start-ollama.ps1` → `status-ollama.ps1`，然后随便问一个问题验证悬浮窗；
2. 投一份 GB 标准（TXT 最省事）到 `ai/standards/inbox/`，跑 `import-standards.ps1`，验证「检索 + 引用卡片」；
3. 找一个 S40 样品走「录入 → 保存 → **回退到已安排** → 改数据 → 重新安排」，看流程回溯面板的时间线；
4. 用 nj001 试一次 `S70→S60` 敏感回退，确认二次确认与原因必填生效；
5. 确认无误后由 GLM 统一提交并同步 `agent/glm → develop → main`。
