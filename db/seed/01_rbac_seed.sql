...
-- =============================================================================
-- AI 助手（feature A，2026-09-17 增量）：目录 12
--   ⚠️ 前端页面（views/ai/kb.vue、views/ai/conversations.vue、views/rollback/index.vue）
--   与 router/routeRegistry.ts 登记属 T05（前端批次），本批（T01）只落权限种子与菜单。
--   `/ai/conversations`（会话审计）当前挂 123 按钮权限 ai:log:view，页面由 T05 提供；
--   为免「点了 404 的死链」，其 menu_type=3（按钮）不生成独立侧栏菜单，仅作权限位。
-- =============================================================================
...
-- =============================================================================
-- 流程回溯（feature B，2026-09-17 增量）：菜单 13
--   权限分级对应 PRD T4：常规回退 rollback:execute；敏感回退（S70→S60）rollback:sensitive；
--   S80/S90 作废·召回 report:void（普通回退被 4102/4103 显式拒绝）。
-- =============================================================================
...
-- R1 样品登记员
--   2026-09-17 补 12/121/122/124（AI 助手 + 标准库）与 13/131/132（流程回溯 + 常规回退）：
--   登记员可纠错本阶段操作，并可用助手查标准/查状态（PRD 场景「样品登记员」）。
...
(3, 94), (3, 941),
(3, 84), (3, 841),
-- 2026-09-17 补 AI 全量（含会话审计 123）与回溯全量（含敏感回退 133、作废/召回 134）：
-- R2 为业务管理员，是敏感回退与报告作废的授权角色（PRD Q3 假设）。
(3, 12), (3, 121), (3, 122), (3, 123), (3, 124),
(3, 13), (3, 131), (3, 132), (3, 133), (3, 134);
-- R3 检验员（2026-09-13 补 63/631 我的检验任务：检验员查看本人待办的入口）
--   2026-09-17 补 12/122/124（AI 助手查标准）与 13/131/132（流程回溯 + 常规回退）：
--   检验员发现录错时可回退到上一步并修改（PRD 场景「检验员」）。
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`) VALUES
