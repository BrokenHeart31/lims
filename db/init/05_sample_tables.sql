...
CREATE TABLE `sample_info` (
  ...
  `audit_by`              VARCHAR(64)  DEFAULT NULL COMMENT '审核人工号（S60→S70 写入，T-701）',
  `audit_at`              DATETIME     DEFAULT NULL COMMENT '审核时间',
  `audit_opinion`         VARCHAR(500) DEFAULT NULL COMMENT '审核意见',
  `sign_by`               VARCHAR(64)  DEFAULT NULL COMMENT '签发人工号（S70→S80 写入，T-701）',
  `sign_at`               DATETIME     DEFAULT NULL COMMENT '签发时间',
  `report_type`           TINYINT      DEFAULT NULL COMMENT '报告类型 1=CMA 2=CMA-CATL（见 common/enums/ReportType，T-702）',
  `report_generated_at`   DATETIME     DEFAULT NULL COMMENT '报告生成时间（S80→S90 时写入）',
  `report_generated_by`   BIGINT       DEFAULT NULL COMMENT '报告生成人 sys_user.id',
